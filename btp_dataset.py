...
class BtpDataset(Dataset):
    ...
    def __init__(self, csv_file, normalize=True):
        ...
        df['Timestamp'] = pd.to_datetime(df["data_column"].map(str) + " " +
                                         df["orario_column"],
                                         dayfirst=True)
        df = df.drop(['data_column', 'orario_column'],
                     axis=1).set_index("Timestamp")
        btp_price = df.BTP_Price
        data = torch.from_numpy(
            np.expand_dims(
                np.array(
                    [group[1] for group in btp_price.groupby(df.index.date)]),
                -1)).float()
        print(data.shape)
    ...
    def sample_deltas(self, number):
        ...
        print(self.delta_mean, self.delta_std)
